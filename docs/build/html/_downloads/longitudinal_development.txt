...
* set the basics
local waves "a b c d e f g h i j k l m n o p q r s t u v w x y z ba bb bc bd be bf bg bh"
local maxwave: word count `waves'
local maxyear=1983+`maxwave' 
 
use \\hume\rdc-gen\consolidated\soep-core\soep.v34\consolidated1\ppfad.dta, replace
...
* recode new nettocodes since v.34
qui foreach i in `waves' { 
replace `i'netto=31 if `i'netto==41
replace `i'netto=30 if inlist(`i'netto,40,41,42,44,48,49)
}
...
graph export  H:\where2.eps, replace 
! convert H:\where2.eps H:\where2.png
