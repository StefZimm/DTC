...
clear all
use *netto psample letztbef using \\hume\rdc-gen\consolidated\soep-core\soep.v34\consolidated1\ppfad.dta
...
local row=1
qui foreach wv in a b c d e f g h i j k l m n o p q r s t u v w x y z ba bb bc bd be bf bg bh{
   forvalues i=1/`cols' {
...
local colnm "A B C D E F G H I J K L1 L2 L3 M1 M2 M3 M4 M5 N"
...
#d;
graph bar (first) A B C D E F G H I J K L1 L2 L3 M1 M2 M3 M4 M5 N, over(year, label(angle(ninety))) stack 
   ...
   bar(19, lcolor(black) lpattern(solid)) bar(20, lcolor(black) lpattern(solid))
	ylabel(, angle(zero)) 
	legend(order(1 "Sample A" 2 "Sample B" 3 "Sample C" 4 "Sample D1/D2" 5 "Sample E" 
	             6 "Sample F" 7 "Sample G" 8 "Sample H" 9 "Sample I" 10 "Sample J"
                     ...
                     16 "Sample M2" 17 "Sample M3" 18 "Sample M4" 19 "Sample M5" 20 "Sample N") 
...
graph export H:\crossdevel.ps, replace
shell convert H:\crossdevel.png, replace
